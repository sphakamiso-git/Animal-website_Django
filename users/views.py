...
from .forms import UserUpdateForm, SetPasswordForm, PasswordResetForm
...
def activateEmail(request,user, to_email):
    ...
    if email.send():
        messages.success(request, f'Dear <b>,<{user}</b>, please go to your email <b>{to_email}</b> inbox and click on \
                     the received activation link to confirm and complete the registration... <b>Note</> Check your spam folder.')
    else:
...
def password_reset_request(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if associated_user:
                ...
                if email.send():
                    messages.success(request, 
                                     ...
                                     you should receive them shortly.<br> If you don't receive an email, 
                                     please make sure you've entered the address you registered ith, and 
            ...
            return redirect('homepage')
        for key, error in list(form.errors.items()):
            if key == 'captcha' and error[0] == 'This field is requered.':
                messages.error(request, 'You must pass the reCAPTCHA test')
                continue 
